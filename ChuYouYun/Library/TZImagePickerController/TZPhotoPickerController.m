...
- (void)configCollectionView {
    ...
    if (iOS7Later) top += 20;
    _collectionView = [[STUICollectionView alloc] initWithFrame:CGRectMake(margin, top, self.view.width - 2 * margin, self.view.height - 50 - top-MACRO_UI_SAFEAREA) collectionViewLayout:layout];
...
- (void)configBottomToolBar {
    UIView *bottomToolBar = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.height-50-MACRO_UI_SAFEAREA, self.view.width, 50+MACRO_UI_SAFEAREA)];
    ...
    if (imagePickerVc.allowPickingOriginalPhoto) {
        ...
        _originalPhotoButton.frame = CGRectMake(50, self.view.height - 50-MACRO_UI_SAFEAREA, 130, 50);
    ...
    if (_singleChoose) {
        ...
        _collectionView.frame = CGRectMake(X(_collectionView), Y(_collectionView),WIDTH(_collectionView), HEIGHT(_collectionView)+50);
    }
...
- (void)okButtonClick {
    ...
    [user setObject:@"no" forKey:@"upLoadOrginImg"];
    [user synchronize];
    TZImagePickerController *imagePickerVc = (TZImagePickerController *)self.navigationController;
    ...
    if (_isSelectOriginalPhoto) {
        ...
        [user setObject:@"yes" forKey:@"upLoadOrginImg"];
        [user synchronize];
    }
    else {
        for (NSInteger i = 0; i < _selectedPhotoArr.count; i++) { [photos addObject:@1];[infoArr addObject:@1]; }
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.singleChoose = _singleChoose;
    return cell;
}
