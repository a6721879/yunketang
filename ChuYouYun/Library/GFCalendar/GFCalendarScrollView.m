...
@property (assign ,nonatomic)long long timeSp;
@property (assign ,nonatomic)long long currtimeSp;
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if ([super initWithFrame:frame]) {
        ...
        NSLog(@"%@",timeSp);
        _currtimeSp = [timeSp integerValue];
    }
...
- (void)getSchoolID:(NSNotification *)not {
...
- (void)setupCollectionViews {
    ...
    flowLayout.itemSize = CGSizeMake(self.bounds.size.width / 7.0, self.bounds.size.width / 7.0 * 1);
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (collectionView == _collectionViewL) {
        ...
        if (indexPath.row >= firstWeekday && indexPath.row < firstWeekday + totalDays) {
            ...
            cell.number.text = _NumberArray[indexPath.row - firstWeekday + 1];
        ...
        if (indexPath.row >= firstWeekday && indexPath.row < firstWeekday + totalDays) {
            ...
            if ((monthInfo.month == [[NSDate date] dateMonth]) && (monthInfo.year == [[NSDate date] dateYear])) {
                if (indexPath.row == [[NSDate date] dateDay] + firstWeekday - 1) {
                    ...
                    cell.number.text = _NumberArray[indexPath.row];
                    NSLog(@"----%@",_NumberArray[indexPath.row]);
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (scrollView.contentOffset.x < self.bounds.size.width) {
        ...
        NSLog(@"%ld",_Number);
        //得到时间戳
        _timeSp = _currtimeSp + (_Number * 3600 * 24 * 30);
        
        NSString *time = [NSString stringWithFormat:@"%lld",_timeSp];
        NSLog(@"%@",time);
