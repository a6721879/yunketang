...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self.collectionView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.bottom.equalTo(self.mas_bottom).offset(-Fit(125));
        }];
...
- (void)minusButtonAction
{
    ...
    if ([self.resultArray containsObject:string]) {
        ...
        NSString *answerString = [NSString stringWithFormat:@"%@:%@",MTLocalized(@"tool.zhengquedaan"),stringABC];
...
- (void)finishButtonAction
{
    ...
    if (!self.resultArray.count) {
        [TKUtil showMessage:MTLocalized(@"tool.leastanswer")];
        return;
    }
    ...
    [[TKRoomManager instance] pubMsg:@"Question" msgID:msgID toID:@"__all" data:json save:YES extensionData:dict2 completion:nil];
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *answerString = [NSString stringWithFormat:@"%@:%@",MTLocalized(@"tool.zhengquedaan"),string];
...
- (UIButton *)addButton
{
    if (!_addButton) {
        ...
        [_addButton setTitle:MTLocalized(@"tool.tianjia") forState:UIControlStateNormal];
...
- (UIButton *)minusButton
{
    if (!_minusButton) {
        ...
        [_minusButton setTitle:MTLocalized(@"tool.shanchu") forState:UIControlStateNormal];
...
- (UIButton *)finishButton
{
    if (!_finishButton) {
        ...
        [_finishButton setTitle:MTLocalized(@"tool.fabudati") forState:UIControlStateNormal];
...
- (UILabel *)label
{
    if (!_label) {
        ...
        _label.sakura.textColor(@"TKToolsBox.answer_text");
        _label.text = MTLocalized(@"tool.zhengquedaan");
    }
    return _label;
}
