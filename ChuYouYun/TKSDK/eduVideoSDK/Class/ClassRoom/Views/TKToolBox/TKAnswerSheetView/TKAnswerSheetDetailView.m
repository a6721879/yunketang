...
- (void)setDict:(NSDictionary *)dict
{
    ...
    self.numLabel.text  = [NSString stringWithFormat:@"%@：%@%@",MTLocalized(@"tool.datirenshu"),num,MTLocalized(@"tool.ren")];
...
- (void)timerAction:(NSTimer *)timer
{
    ...
    NSString *toID         = [TKRoomManager instance].localUser.peerID;
    //toID
    [[TKRoomManager instance] pubMsg:@"GetQuestionCount" msgID:msgID toID:toID data:@{} save:NO extensionData:dict2 completion:nil];
}
...
- (void)finishButtonAction
{
    ...
    if (self.finishButton.selected) {
        ...
        //发送结束信令
        [[TKRoomManager instance] delMsg:@"Question" msgID:msgID toID:@"__all" data:@{} completion:nil];
        [[TKRoomManager instance] delMsg:@"PublishResult" msgID:@"PublishResult" toID:@"__all" data:@{} completion:nil];
        
        //重新开始
        [[TKRoomManager instance] pubMsg:@"Question"
                                   msgID:msgID
                                    toID:@"__all"
                                    data:@"{\"action\":\"open\"}"
                                    save:YES
        ...
        [[TKRoomManager instance] pubMsg:@"AnswerDrag"
                                   msgID:@"AnswerDrag"
                                    toID:sTellAllExpectSender
        ...
        [[TKRoomManager instance] pubMsg:@"Question" msgID:msgID toID:@"__all" data:json1 save:YES extensionData:dict1 completion:^(NSError *error) {
        }];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.numLabel.text = [NSString stringWithFormat:@"%d%@",[result[indexPath.row] intValue],MTLocalized(@"tool.ren")];
...
- (UIButton *)detailButton
{
    if (!_detailButton) {
        ...
        [_detailButton setTitle:MTLocalized(@"tool.detail") forState:UIControlStateNormal];
...
- (UIButton *)finishButton
{
    if (!_finishButton) {
        ...
        [_finishButton setTitle:MTLocalized(@"tool.restart") forState:UIControlStateSelected];
...
- (UILabel *)timeLabel
{
    if (!_timeLabel) {
        ...
        _timeLabel.text = [NSString stringWithFormat:@"%@：00:00:00",MTLocalized(@"tool.time")];
    }
    return _timeLabel;
}
