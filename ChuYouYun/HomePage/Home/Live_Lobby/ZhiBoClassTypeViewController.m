//
//  ZhiBoClassTypeViewController.m
//  dafengche
//
//  Created by 赛新科技 on 2017/8/16.
...
@interface ZhiBoClassTypeViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation ZhiBoClassTypeViewController
...
- (void)interFace {
    ...
    //添加通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(removeSelfView) name:@"ZhiBoClassTypeRemove" object:nil];
}
...
- (void)addTabView {
    ...
    _oneTableView = [[STTableView alloc] initWithFrame:CGRectMake(0, 0, MainScreenWidth, _tabView.height) style:UITableViewStyleGrouped];
    ...
    _twoTableView = [[STTableView alloc] initWithFrame:CGRectMake(tableViewW + MainScreenWidth, 0, tableViewW, _tabView.height) style:UITableViewStyleGrouped];
    ...
    _thereTableView = [[STTableView alloc] initWithFrame:CGRectMake(tableViewW * 2 + MainScreenWidth, 0, tableViewW, _tabView.height) style:UITableViewStyleGrouped];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (_oneTableView == tableView) {//点击一级分类的时候
        if (indexPath.row == 0) {//点击全部
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:@"NSNotificationZhiBoClassTypeID" object:dict];
            [self removeSelfView];
        } else {
...
#pragma mark --- 通知
-(void)removeSelfView {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"NSNotificationZhiBoClassTypeButton" object:nil];
