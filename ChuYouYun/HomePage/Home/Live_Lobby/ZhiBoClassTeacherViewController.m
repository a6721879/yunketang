...
- (void)addTabView {
    ...
    _tableView = [[STTableView alloc] initWithFrame:CGRectMake(0, 0, MainScreenWidth, MainScreenHeight - 109 - 200) style:UITableViewStyleGrouped];
...
{
    Number = Number + 1;
    [self netWorkLiveGetScreenTeacherMoreData];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
- (void)netWorkLiveGetScreenTeacher {
    ...
    [mutabDict setObject:@(Number) forKey:@"page"];
...
- (void)netWorkLiveGetScreenTeacherMoreData {
