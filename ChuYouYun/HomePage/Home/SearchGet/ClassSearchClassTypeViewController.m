//
//  ClassSearchClassTypeViewController.m
//  dafengche
//
...
@interface ClassSearchClassTypeViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation ClassSearchClassTypeViewController
...
- (void)interFace {
    ...
    //添加通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(removeSelfView) name:@"SearchClassTypeRemove" object:nil];
}

- (void)addAllView {
    _tabView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, MainScreenWidth, MainScreenHeight - MACRO_UI_UPHEIGHT - 45 - ([_typeTagStr integerValue] == 1 ? MACRO_UI_TABBAR_HEIGHT : 0))];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_oneTableView == tableView) {//点击一级分类的时候
        if (indexPath.row == 0) {//点击全部
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:@"NSNotificationSearchClassTypeID" object:dict];
            [self removeSelfView];
        } else {
...
#pragma mark --- 通知
-(void)removeSelfView {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"NSNotificationSearchClassTypeButton" object:nil];
