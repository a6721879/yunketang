//
//  Good_InstClassViewController.m
//  dafengche
//
...
@interface Good_InstClassViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation Good_InstClassViewController
...
- (void)interFace {
    ...
    //添加通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(removeSelfView) name:@"InstClassRemove" object:nil];
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (_oneTableView == tableView) {//点击一级分类的时候
        ...
        if ([[_dataArray objectAtIndex:indexPath.row] arrayValueForKey:@"child"] == nil) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:@"NSNotificationInstClassTypeID" object:dict];
            [self removeSelfView];
            return;
        }
...
#pragma mark --- 通知
-(void)removeSelfView {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"NSNotificationInstClassTypeButton" object:nil];
