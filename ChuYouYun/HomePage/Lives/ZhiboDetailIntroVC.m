...
@interface ZhiboDetailIntroVC ()<UIScrollViewDelegate,ZLPhotoPickerBrowserViewControllerDelegate,ZLPhotoPickerBrowserViewControllerDataSource, WKNavigationDelegate> {
...
@property (strong ,nonatomic) YKTWebView *TeacherIntroWeb;
// 提取标签里面纯文字(如果web高度大于某个值,那么收起来的时候就显示纯文字,展开就显示完整版web)
...
- (void)makeTeacherIntroView {
    ...
    _TeacherIntroWeb = [[YKTWebView alloc] initWithFrame:CGRectMake(0, _teacherHeaderImage.bottom + 10 * HigtEachUnit, MainScreenWidth, 1)];
...
-(void)handleSingleTap1:(UITapGestureRecognizer *)sender
{
    if (sender.view == _TeacherIntroWeb) {
        ...
        [_TeacherIntroWeb evaluateJavaScript:imgStr completionHandler:^(NSString *urlToSave, NSError * _Nullable error) {
            NSString *orignalStr = [NSString stringWithFormat:@"document.elementFromPoint(%f, %f).getAttribute('_src')", pt.x, pt.y];
            [_TeacherIntroWeb evaluateJavaScript:orignalStr completionHandler:^(NSString *orignalUrl, NSError * _Nullable error) {
        ...
        NSString *imgStr = [NSString stringWithFormat:@"document.elementFromPoint(%f, %f).src", pt.x, pt.y];
        [_ClassIntroWeb evaluateJavaScript:imgStr completionHandler:^(NSString *urlToSave, NSError * _Nullable error) {
            NSString *orignalStr = [NSString stringWithFormat:@"document.elementFromPoint(%f, %f).getAttribute('_src')", pt.x, pt.y];
            [_ClassIntroWeb evaluateJavaScript:orignalStr completionHandler:^(NSString *orignalUrl, NSError * _Nullable error) {
                if (urlToSave.length > 0&&[urlToSave rangeOfString:@"/expression/"].location==NSNotFound&&[urlToSave rangeOfString:@"/emotion/"].location==NSNotFound) {
                    _imageBigUrl = orignalUrl;
                    _imageSmallUrl = urlToSave;
                    isShowImageTouch = YES;
