...
- (void)viewDidLoad {
    ...
    self.titleArray = @[@[@"资讯"],@[@"考试"],@[@"问答",@"卡券"],@[@"套餐"]];
    self.imageArray = @[@[@"资讯"],@[@"考试(3)"],@[@"问答(4)",@"卡券(3)"],@[@"发现_小组"]];
}
...
//        return 2;
//    }
    NSArray *list = self.titleArray[section];
    return list.count;
}
...
//    NSArray *imageArray = @[@"资讯",@"商 城 (2)",@"考试(3)",@"文库",@"问答(4)",@"卡券(3)",@"发现_小组"];
    ...
    NSArray *listTitles = self.titleArray[indexSet];
    NSString *titleStr = listTitles[indexRow];
    
    NSArray *listImages = self.imageArray[indexSet];
    NSString *imageStr = listImages[indexRow];

    [cell.SYGButton setBackgroundImage:[UIImage imageNamed:imageStr] forState:UIControlStateNormal];
    cell.SYGLabel.text = titleStr;
    
//    if (indexSet == 0) {
//
//    } else if (indexSet == 1) {
//        [cell.SYGButton setBackgroundImage:[UIImage imageNamed:self.imageArray[indexRow + 1]] forState:UIControlStateNormal];
//        cell.SYGLabel.text = self.titleArray[indexRow+ 1];
//    } else if (indexSet == 2) {
//        [cell.SYGButton setBackgroundImage:[UIImage imageNamed:self.imageArray[indexRow + 2]] forState:UIControlStateNormal];
//        cell.SYGLabel.text = self.titleArray[indexRow + 2];
//    } else if (indexSet == 3) {
//        [cell.SYGButton setBackgroundImage:[UIImage imageNamed:self.imageArray[indexRow + 3]] forState:UIControlStateNormal];
//        cell.SYGLabel.text = self.titleArray[indexRow + 4];
//    }
