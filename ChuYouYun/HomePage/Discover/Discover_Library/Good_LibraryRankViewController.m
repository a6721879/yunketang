//
//  Good_LibraryRankViewController.m
//  YunKeTang
//
...
@interface Good_LibraryRankViewController ()<UITableViewDelegate,UITableViewDataSource> {
    NSInteger Number;
}
...
@implementation Good_LibraryRankViewController
...
- (void)interFace {
    ...
    _dataArray = @[@"综合排序",@"精华排序",@"热门排序"];
    
    _typeArray = @[@"0",@"1",@"hot"];
    
    //添加通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(removeSelfView) name:@"Good_LibraryRankRemove" object:nil];
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"NSNotificationGood_LibraryRankID" object:dict];
    [self removeSelfView];
}

#pragma mark --- 通知
-(void)removeSelfView {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"NSNotificationGood_LibraryRankButton" object:nil];
