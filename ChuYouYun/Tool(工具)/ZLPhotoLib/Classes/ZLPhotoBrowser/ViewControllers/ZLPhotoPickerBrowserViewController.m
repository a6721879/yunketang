...
#warning 添加了一个保存按钮
        if (!_saveBtn) {
            _saveBtn = [[UIButton alloc]initWithFrame:CGRectMake(20, MainScreenHeight-46, 25, 25)];
            [_saveBtn setBackgroundImage:[UIImage imageNamed:@"ic_savepic"] forState:0];
            _saveBtn.autoresizingMask = UIViewAutoresizingFlexibleHeight;
...
-(void)saveImage
{
    UIImage *seleImg = [[SDImageCache sharedImageCache] imageFromDiskCacheForKey:[((ZLPhotoPickerBrowserPhoto *)self.photos[_currentPage]).photoURL absoluteString]];
    if (!seleImg) {
        seleImg = ((ZLPhotoPickerBrowserPhoto *)self.photos[_currentPage]).thumbImage;
    }
    if([UIImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypeSavedPhotosAlbum]) {

        UIImageWriteToSavedPhotosAlbum(seleImg, nil, nil, nil);
        if (seleImg) {
            ShowMissHid(@"保存成功");
        }
    }else{
        if (seleImg) {
            ShowMissHid(@"没有用户权限,保存失败");
        }
    }
}
...
//        [deleleBtn setImage:[UIImage ml_imageFromBundleNamed:@"nav_delete_btn"] forState:UIControlStateNormal];
        [deleleBtn setImage:[UIImage imageNamed:@"icon_trashcan"] forState:UIControlStateNormal];
...
- (void)showToView{
    ...
    __block BOOL isHomeImg = _homeImg;
    self.disMissBlock = ^(NSInteger page){
        ...
        // 缩放动画
        if(weakSelf.status == UIViewAnimationAnimationStatusZoom&&!_swDismiss){
            UIImage *thumbImage = nil;
